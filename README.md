...
- Earley charts can only be walked right-to-left, despite being built left-to-right. For the sake of learnability, my implementation has an arbitrary, right-to-left disambiguation strategy. This is OK for languages where ambiguity is an accident instead of a feature. If you need to fix it, my blog posts cover how to get left-to-right disambiguation with specific disambiguation rules. Basically, both of the following (not just one or the other):
  - The earley chart can only be safely walked right-to-left. If you need left-to-right disambiguation, you'll need to reverse the grammar and input token stream before parsing (in code). If the parse fails, unreverse them and parse again before producing error messages. This is simpler than the alternative of copy/pasting the entire Earley algorithm and writing a right-to-left version that produces a left-to-right chart.
    - This is a known problem: [Loup proposes reversing the chart](https://loup-vaillant.fr/tutorials/earley-parsing/parser) instead of the grammar and token list, and this definitely works for most grammars, but it's fragile and easy to implement wrong. I recommend the dual reversal method.
